...
- **All cert logic is client-side** (`src/core.js`, `pkijs` + WebCrypto, bundled to `public/app.js` by `build.mjs`). The Worker (`src/worker.js`) is a thin relay: `/proxy` fetches the certificate's own `http(s)` AIA/CRL/OCSP (a browser can't — CORS + mixed content), and `/chain?host=` reads a host's *presented* chain via a hand-rolled TLS 1.2 ClientHello over `cloudflare:sockets` (`src/tls.js`) — `fetch()` never exposes the peer chain. Forces TLS 1.2 (omits supported_versions) to read the cleartext Certificate message; TLS 1.3-only hosts can't be scanned (paste instead). Keep the SSRF guard (`isBlockedHost`) on both routes — generic (any public host), not an allowlist.
